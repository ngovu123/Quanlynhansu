    ...
    public class TonGiaoController : Controller
    {
        ...
        public ActionResult Index(int? page, string searchString)
        {
            ...
            ViewBag.list = new List<lookfor>() {
                ...
                new lookfor(){ID="1",Name="Mã tôn giáo"},

                new lookfor(){ID="2",Name="Tên tôn giáo"},
            ...
            if (TempData["list"] == null)
            {
                ...
                var dantoc = from s in db.TONGIAOs select s;
                int PageNum = (page ?? 1);
                int PageSize = 5;
                return View(dantoc.ToList().OrderBy(n => n.MATG).ToPagedList(PageNum, PageSize));
            ...
            if (TempData["list"].ToString() == "1")
            {
                ...
                if (!String.IsNullOrEmpty(searchString))
                {
                    ...
                    dantoc = dantoc.Where(b => b.MATG.ToString().ToLower().Contains(searchString));
                }
                int PageNum = (page ?? 1);
                int PageSize = 5;
                return View(dantoc.ToList().OrderBy(n => n.MATG).ToPagedList(PageNum, PageSize));
            ...
            else if (TempData["list"].ToString() == "2")
            {
                ...
                {
                    searchString = searchString.ToLower();
                    dantoc = dantoc.Where(b => b.TENTG.ToLower().Contains(searchString));
                }
                int PageNum = (page ?? 1);
                int PageSize = 5;
                return View(dantoc.ToList().OrderBy(n => n.MATG).ToPagedList(PageNum, PageSize));
        ...
        public ActionResult Export()
        {
            ...
            {
                var worksheet = package.Workbook.Worksheets.Add("tongiao");

                // Add headers
                worksheet.Cells[1, 1].Value = "ID";
                worksheet.Cells[1, 2].Value = "TÊN TÔN GIÁO";
                ...
                // Add data
                var ex = db.TONGIAOs.ToList();
                int row = 2;
                foreach (var employee in ex)
                {
                    worksheet.Cells[row, 1].Value = employee.MATG;
                    worksheet.Cells[row, 2].Value = employee.TENTG;
        ...
        public ActionResult Details(int? id)
        {
            ...
            TONGIAO tONGIAO = db.TONGIAOs.Find(id);
            if (tONGIAO == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Create([Bind(Include = "MATG,TENTG")] TONGIAO tONGIAO)
        {
            if (ModelState.IsValid)
            {
                db.TONGIAOs.Add(tONGIAO);
                db.SaveChanges();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "MATG,TENTG")] TONGIAO tONGIAO)
        {
            if (ModelState.IsValid)
            {
                db.Entry(tONGIAO).State = EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(tONGIAO);
